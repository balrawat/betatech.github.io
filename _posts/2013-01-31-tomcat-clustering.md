...
For more information about the how and why of clustered architecture, check out our [Tomcat Clustering] Tomcat Clustering Basics article for an in-depth look at all the components of a cluster, comparisons of different approaches, and more.
...
If you haven't already installed them, the first thing to do is to download and install the latest stable versions of Apache Tomcat 6.x and Apache HTTPD. You can find the latest stable versions on the [Apache HTTPD] Apache HTTPD and [Tomcat] Tomcat project sites.
...
For the purposes of this tutorial, we'll assume that you understand how to install these components. If you need additional help, don't worry - the Apache HTTPD [installation documentation] installation documentation is quite good.
...
As this is a bare-bones example configuration, we haven't configured any non-required directives, but as with the worker directives, the full list is available on the Tomcat Connectors [project site] project site, and is recommended reading.
...
If you'd like to learn more about load balancing with mod_jk, visit the [load balancing how-to article] load balancing how-to article on the Apache site.
